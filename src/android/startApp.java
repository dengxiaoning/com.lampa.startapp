...
public class startApp extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        else if (action.equals("downloadApps")){
        	 this.downloadApps(args, callbackContext);
        }
        return true;
    }
	...
	public void downloadApps(JSONArray args, CallbackContext callback){
		...
		try {
			if(args.get(0) instanceof JSONObject){
				params = args.getJSONObject(0);
				if(params.has("application")) {
					Uri uri = Uri.parse("market://details?id="+params.getString("application")+"");//id为包名 
